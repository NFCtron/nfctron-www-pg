...
const supportItems = [
  { title: 'Vrátit zbývající kredit', text: 'Pošlete nevyužité peníze z čipu zpět na bankovní účet.', href: 'https://support.nfctron.com/cs/clanek/jak-si-vratit-zbyvajici-kredit-nfctron' },
  { title: 'Dobít kredit', text: 'Připravte si placení na podporované akci ještě před jejím začátkem.', href: 'https://support.nfctron.com/cs/clanek/jak-si-dobiju-kredit' },
  { title: 'Vstupenka a QR kód', text: 'Zjistěte, kde vstupenku najdete a zda ji potřebujete tisknout.', href: 'https://support.nfctron.com/cs/clanek/tisk-vstupenky-nfctron' },
  { title: 'Ztracený QR kód', text: 'Poradíme, jak znovu získat přístup k online účtence a čipu.', href: 'https://support.nfctron.com/cs/clanek/ztrata-tisteneho-qr-kodu' },
];
...
function SupportIcon() {
  return <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M4 13a8 8 0 0 1 16 0"/><path d="M4 13v4a2 2 0 0 0 2 2h1v-7H4ZM20 13v4a2 2 0 0 1-2 2h-1v-7h3ZM17 19c0 1.1-.9 2-2 2h-3"/></svg>;
}
      ...
      <article className="overflow-hidden rounded-xl border border-black/5 bg-white transition duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-primary-900/10">
        ...
        <div className="relative p-4">
          <div className="pr-14">
            ...
            {event.features?.length ? <div className="mt-3 flex flex-wrap gap-1.5">{event.features.slice(0, 2).map(feature => <span key={feature} className="inline-flex items-center gap-1.5 rounded-full bg-primary-50 px-2.5 py-1 text-[9px] font-semibold text-primary-700"><span className="h-1.5 w-1.5 rounded-full bg-primary-500" />{t(feature)}</span>)}</div> : null}
            <p className="mt-4 flex items-center gap-2 text-xs text-gray-500"><CalendarIcon className="h-3.5 w-3.5 shrink-0" />{t(event.date)}</p>
            <p className="mt-2 flex items-center gap-2 text-xs text-gray-500"><span>⌖</span>{t(event.place)} · {t(event.city)}</p>
          </div>
...
function PromoCard({ item, t, year = false }: { item: { title: string; meta: string; accent: string; action?: string; image?: string }; t: Translator; year?: boolean }) {
  return <article className="group overflow-hidden rounded-xl border border-gray-200 bg-white transition hover:-translate-y-1 hover:shadow-xl hover:shadow-primary-900/10">
    ...
    <div className="flex items-center justify-between gap-4 p-5">
      <div><h3 className="text-lg font-bold text-primary-900">{item.title}</h3><p className="mt-1 text-xs text-gray-500">{t(item.meta)}</p></div>
      <button className="shrink-0 rounded-full border border-primary-500 px-4 py-2 text-xs font-semibold text-primary-700 transition hover:bg-primary-50">{t(item.action ?? 'Detail akce')}</button>
    </div>
  </article>;
}
      ...
      <header className="sticky top-0 z-50 border-b border-gray-200/80 bg-white/95 backdrop-blur-md">
        <nav className="container-fluid flex h-14 items-center justify-between gap-6">
          ...
          <div className="hidden h-full items-center gap-7 md:flex">
            <a href="#home" aria-current="page" className="flex h-full items-center border-b border-primary-700 pt-px text-xs font-semibold text-primary-700">{t('Domů')}</a>
            <a href="#events" className="flex h-full items-center border-b border-transparent pt-px text-xs font-medium text-gray-500 transition hover:text-primary-700">{t('Akce')}</a>
            <a href="#how-it-works" className="flex h-full items-center border-b border-transparent pt-px text-xs font-medium text-gray-500 transition hover:text-primary-700">{t('Jak to funguje')}</a>
            <Link href="/for-organizers" className="flex h-full items-center border-b border-transparent pt-px text-xs font-medium text-gray-500 transition hover:text-primary-700">{t('Pro pořadatele')}</Link>
          </div>
          <div className="flex items-center gap-1.5"><LanguageMenu locale={locale} /><a href="#support" aria-label="Support" className="flex h-8 items-center gap-1.5 rounded-full px-2 text-[11px] font-semibold text-gray-500 transition hover:bg-gray-100 hover:text-primary-700"><SupportIcon /><span className="hidden lg:inline">Support</span></a><Link href="https://tickets.nfctron.com/login" className="rounded-full bg-primary-700 px-4 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-primary-800">{t('Přihlásit se')}</Link></div>
        </nav>
      </header>
      ...
      <main>
        <section id="home" className="bg-white pb-8 pt-8 sm:pb-8 sm:pt-10 lg:pb-8 lg:pt-12">
          <div className="container-fluid grid gap-8 lg:grid-cols-[.9fr_1.1fr] lg:items-start">
            <div className="max-w-xl">
              <p className="mb-4 text-xs font-bold uppercase tracking-[.16em] text-primary-600">{t('Vstupenky a placení na akcích')}</p>
        ...
        <section className="bg-white pb-12 pt-8 sm:pb-14 sm:pt-9 lg:pb-16 lg:pt-10">
          <div className="container-fluid">
            <div className="flex items-end justify-between gap-6"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-primary-600">{t('Více míst, jeden zážitek')}</p><h2 className="mt-3 text-3xl font-bold tracking-[-.035em] text-primary-900">{t('Sériové akce')}</h2></div><button className="hidden text-sm font-semibold text-primary-700 sm:block">{t('Zobrazit všechny')} →</button></div>
        ...
        <section className="bg-primary-900 py-12 text-white sm:py-14 lg:py-16">
          <div className="container-fluid">
            <div className="max-w-2xl"><p className="text-xs font-bold uppercase tracking-[.16em] text-primary-300">{t('Příští sezóna začíná teď')}</p><h2 className="mt-3 text-3xl font-bold tracking-[-.035em] sm:text-4xl">{t('Akce, na které se těšíme v roce 2027')}</h2><p className="mt-4 text-sm leading-relaxed text-white/55">{t('První festivaly a koncerty jsou v prodeji. Vyberte si včas a mějte příští léto naplánované.')}</p></div>
        ...
        <section className="border-y border-gray-100 bg-[#faf9ff] py-12 sm:py-14 lg:py-16">
          <div className="container-fluid">
            <div className="max-w-2xl"><p className="text-xs font-bold uppercase tracking-[.16em] text-primary-600">{t('Vše důležité pro návštěvníky')}</p><h2 className="mt-3 text-3xl font-bold tracking-[-.035em] text-primary-900">{t('NFCtron drží celý zážitek pohromadě.')}</h2></div>
            <div className="mt-9 grid gap-px overflow-hidden rounded-xl border border-gray-200 bg-gray-200 md:grid-cols-3">
              {[['01', 'Vstupenka vždy po ruce', 'Nákup online a rychlý vstup na akci bez hledání e-mailů.'], ['02', 'Rychlé placení', 'Čip, karta nebo telefon. Jednoduše u stánku i bez hotovosti.'], ['03', 'Vrácení kreditu', 'Nevyužité peníze po akci odešlete zpět přímo online.']].map(([number, title, text]) => <div key={number} className="bg-white p-7"><span className="text-xs font-bold text-primary-500">{number}</span><h3 className="mt-7 text-lg font-bold text-primary-900">{t(title)}</h3><p className="mt-2 text-sm leading-relaxed text-gray-500">{t(text)}</p></div>)}
            </div>
          </div>
        </section>
        ...
        <section className="border-t border-gray-100 bg-[#faf9ff] py-12 sm:py-14 lg:py-16">
          <div className="container-fluid">
            <div className="flex items-end justify-between gap-6"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-primary-600">{t('Novinky a inspirace')}</p><h2 className="mt-3 text-3xl font-bold tracking-[-.035em] text-primary-900">{t('Blog NFCtron')}</h2></div><Link href="https://www.nfctron.com/cs/blog" className="hidden text-sm font-semibold text-primary-700 sm:block">{t('Všechny články')} →</Link></div>
          ...
          <div className="container-fluid">
            <div className="grid gap-6 lg:grid-cols-[.7fr_1.3fr] lg:items-start">
              <div className="max-w-md">
                <p className="text-xs font-bold uppercase tracking-[.16em] text-primary-600">NFCtron Support</p>
                <h2 className="mt-3 text-3xl font-bold tracking-[-.035em] text-primary-900">{t('Máte vstupenku nebo jste právě po akci?')}</h2>
                ...
                <Link href="https://support.nfctron.com/article/43-pro-navstevniky" className="btn-primary mt-5">{t('Otevřít veškerou nápovědu')} <ArrowIcon /></Link>
              </div>
              <div className="grid gap-3 sm:grid-cols-2">
                {supportItems.map((item, index) => <Link key={item.title} href={item.href} className="group rounded-xl border border-gray-200 bg-white p-4 transition hover:border-primary-200 hover:bg-primary-50 hover:shadow-lg hover:shadow-primary-900/5"><div className="flex items-start justify-between gap-4"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-primary-100 text-[9px] font-bold text-primary-700">0{index + 1}</span><span className="text-gray-300 transition group-hover:translate-x-0.5 group-hover:text-primary-600">→</span></div><h3 className="mt-4 text-sm font-bold text-primary-900">{t(item.title)}</h3><p className="mt-1.5 text-[11px] leading-relaxed text-gray-500">{t(item.text)}</p></Link>)}
              </div>
            </div>
          </div>
        </section>
        ...
        <section id="how-it-works" className="section bg-white">
          <div className="container-fluid text-center">
            <p className="mb-3 text-xs font-bold uppercase tracking-[.16em] text-primary-600">{t('NFCtron na akcích')}</p>
            ...
            <div className="mt-10 grid gap-px overflow-hidden rounded-xl border border-gray-200 bg-gray-200 text-left md:grid-cols-3">
              {[['01', 'Kupte vstupenku', 'Bezpečně online, během několika minut a rovnou do vašeho účtu.'], ['02', 'Plaťte bez starostí', 'Čipem, kartou nebo mobilem. Rychle i tam, kde není internet.'], ['03', 'Vraťte si kredit', 'Zbytek peněz po akci pošlete zpět na účet přímo z aplikace.']].map(([number, title, copy]) => <div key={number} className="bg-white p-7"><span className="text-xs font-bold text-primary-500">{number}</span><h3 className="mt-6 text-xl font-bold text-primary-900">{t(title)}</h3><p className="mt-3 text-sm leading-relaxed text-gray-500">{t(copy)}</p></div>)}
            </div>
          </div>
        </section>
